// PROBLEM:

// You are given an array prices where 'prices[i]' is the price of a given stock on the 'i'th day.
// You want to maximize your profit by choosing a single day to buy one stock
// and choosing a different day in the future to sell that stock.
// Return the maximum profit you can achieve from this transaction. If you cannot achieve any profit, return 0.

// EXAMPLE:

// Input: prices = [7,1,5,3,6,4]
// Output: 5
// Explanation: Buy on day 2 (price = 1) and sell on day 5 (price = 6), profit = 6-1 = 5.
// Note that buying on day 2 and selling on day 1 is not allowed because you must buy before you sell.

// Input: prices = [7,6,4,3,1]
// Output: 0
// Explanation: In this case, no transactions are done and the max profit = 0.

function buyAndSellStock(prices: number[]): number {
    // initializing variables
    let profit: number = 0;
    let priceDifference: number = 0
    let buyingPrice: number, sellingPrice: number;
    
    for (let i: number = 0; i < (prices.length - 1); i++) {
        // console.log(`Now checking ${prices[i]} as first price`);
        for (let j: number = (i+1); j < prices.length; j++) {
            // console.log(`Now checking ${prices[j]} as second price`);
            if (prices[i] < prices[j]) {
                priceDifference = prices[j] - prices[i];
                // console.log(`Price difference between ${prices[j]} and ${prices[i]} is ${priceDifference}`);
            }
            if (priceDifference > profit) {
                profit = priceDifference;
                // console.log(`Current profit is ${profit}`);
            }
        }
    }
    return profit;
}

console.log(buyAndSellStock([7,1,5,3,6,4]));
console.log(buyAndSellStock([7,6,4,3,1]));